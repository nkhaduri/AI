...
def dfs(v, visited, features):
    """traverses all the non visited pixels reachable from given one"""
    visited[v] = True

    x = int(v / DIGIT_DATUM_WIDTH)
    y = v % DIGIT_DATUM_WIDTH
    for i in range(-1, 2):
        for j in range(-1, 2):
            if 0 <= x + i < DIGIT_DATUM_HEIGHT and 0 <= y + j < DIGIT_DATUM_WIDTH and \
                    not visited[(x + i) * DIGIT_DATUM_WIDTH + y + j] and features[(x, y)] == features[(x + i, y + j)]:
                dfs((x + i) * DIGIT_DATUM_WIDTH + y + j, visited, features)
...
def enhancedFeatureExtractorDigit(datum):
    ...
    I counted connected components in the picture,
    given that from white pixels you can only move to adjacent white pixels and
    from gray/black pixels, to adjacent gray/black pixels.
    this gives how many circles are there + 2,
    so I subtract 2 from this count when writing in features
    ##
    """
    features = basicFeatureExtractorDigit(datum)

    visited = []
    for i in xrange(DIGIT_DATUM_WIDTH * DIGIT_DATUM_HEIGHT):
        visited.append(False)

    connectedComponents = 0
    # call dfs from every pixel not yet visited
    for i in xrange(DIGIT_DATUM_WIDTH * DIGIT_DATUM_HEIGHT):
        if not visited[i]:
            dfs(i, visited, features)
            connectedComponents += 1 # found new connected component

    features['conn'] = connectedComponents - 2
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    features['minFoodDist'] = 1./minFoodDist # the closer the food, the higher feature value
    ...
    for i in xrange(len(newGhostStates)):
        ...
        if newScaredTimes[i] >= manDistToGhost > 0:  # if pacman can eat i-th ghost
            features['canEat'] = 1./manDistToGhost # the closer the scared ghost, the higher feature value
        if 0 < manDistToGhost <= 3: # if ghost is close enough
            features['ghostClose'] = 1./manDistToGhost # the closer the ghost, the higher feature value
        if manDistToGhost == 0:
            # if given action will lead us straight at the ghost, it's either really good or really bad
            # depends on weights, which depends on agent we're studying from
            features['ghostClose'] = 10
    return features
