...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            if noisyDistance is None: # if ghost is caught belief for every position but jail should be 0
                allPossible[p] = 0.
            else:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    allPossible[p] = emissionModel[trueDistance] * self.beliefs[p] # update belief
        if noisyDistance is None: # if ghost is caught belief for jail should be 1
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob * self.beliefs[p] # belief for ghost moving in position newPos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in xrange(self.numParticles):
            # distribute particles evenly over legal positions
    ...
    def observe(self, observation, gameState):
        ...
        weights = util.Counter() # weights for each particle
        if noisyDistance is None: # if ghost is caught, should move all the particles to jail
            for i in xrange(self.numParticles):
                self.particles[i] = self.getJailPosition()
        else:
            for p in self.particles: # count weights for each particle
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    weights[p] += emissionModel[trueDistance]

            if weights.totalCount() == 0: # all the weight are zero, so should recreate them
                self.initializeUniformly(gameState)
            else:
                for i in xrange(self.numParticles): # sample each particle from weights distribution
    ...
    def elapseTime(self, gameState):
        ...
        for i in xrange(self.numParticles):
            ...
            self.particles[i] = util.sample(newPosDist) # resample particle from new position distribution
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particles:
            # for every particle present in some position, this positions' belief is increased by 1/numParticles
            beliefs[p] += 1./self.numParticles
        return beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        self.particles = []
        # get cartesian product of numGhosts number of legalPositions
        prod = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(prod) # shuffle product in order to to ensure even placement across the board
        for i in xrange(self.numParticles):
            self.particles.append(prod[i % len(prod)]) # get particles from shuffled product
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            weight = 1 # weight for particle p
            ...
            weights[p] += weight # weight for position of particle p

        if weights.totalCount() == 0: # all the weights are zero, so should recreate particles
            self.initializeParticles()
        else:
            for i in xrange(self.numParticles): # sample each particle from weights distribution
                self.particles[i] = util.sample(weights)
                for j in xrange(self.numGhosts): # check for caught ghosts
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in xrange(self.numGhosts):
                ...
                newParticle[i] = util.sample(newPosDist) # resample particle's i-th ghost from new position distribution
